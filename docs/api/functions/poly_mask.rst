...
.. py:function:: poly_mask(fs, lats, lons, missing=False)
   ...
   *New in Metview version 5.16.0*.
   ...
   The polygons are automatically closed up if the first and last coordinates are not the same. If multiple polygons are specified the output mask is generated by forming their union.
